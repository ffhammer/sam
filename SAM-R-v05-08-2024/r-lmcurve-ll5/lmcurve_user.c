...
typedef struct {
    const double* t;
    const double* y;
    double (*f)(const double t, const double* par, const void* user_data);
   	const void* user_data;
} lmcurve_user_data_struct;

void lmcurve_user_evaluate(
    const double* par, const int m_dat, const void* data, double* fvec,
    int* info)
{
    lmcurve_user_data_struct* D = (lmcurve_user_data_struct*)data;
    int i;
    for (i = 0; i < m_dat; i++)
        fvec[i] = D->y[i] - D->f(D->t[i], par, D->user_data);
}
...
{
    lmcurve_user_data_struct data = { t, y, f, user_data };

    lmmin(n_par, par, m_dat, (const void*)&data, lmcurve_user_evaluate,
          control, status);
}
