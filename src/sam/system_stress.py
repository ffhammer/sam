...
class CleanedPred:
    ...
    def plot(self, title : Optional[str] = None) -> Figure:
        ...
        ax1.plot(self.result.main_fit.concentrations, self.original_fit.survival_curve, label = "Original", linestyle = "--", c = "black")
        ax2.plot(self.result.main_fit.concentrations, self.original_fit.stress_curve, label = "Original", linestyle = "--", c = "black")
        ax1.scatter(pad_c0(self.original_series.concentration), self.original_series.survival_rate, label = "Original", linestyle = "--", c = "black")
        ax2.legend()
        return fig
...
def predict_with_hormesis_cancelled(
    ...
    if hormesis_index is None or hormesis_index < 0 or hormesis_index >= len(main_series.concentration):
