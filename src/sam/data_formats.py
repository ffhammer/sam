...
class ExperimentData:
    ...
    def to_markdown_table(self):
        cols = {
            "Concentration": self.main_series.concentration,
        ...
        non_dup_meta = {
            key: val
            for key, val in cur_meta.items()
            if key not in yaml_meta and val is not None
        }

        if len(non_dup_meta):
            yaml_text = "\n".join(
                [f"{key}: '{val}'" for key, val in non_dup_meta.items()]
            )
            yam_template = "```yaml\n{}\n```\n"
            markdown_text += "\n\nSpecific Settings:\n\n" + yam_template.format(
                yaml_text
...
def load_files(
    ...
    if len(paths) == 0:
        raise ValueError(
            f"Cand find any experiments at {data_dir}. You can control this via the data_dir argument. "
        )
