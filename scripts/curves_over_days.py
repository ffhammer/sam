...
def difference_plots(path):
    series: list[DoseResponseSeries] = [
        read_data(s).main_series for s in glob(str(path / "*.xlsx"))
    ]
    ...
    for s in series:
        scatter = plt.scatter(
            s.concentration, s.survival_rate, label=f"{s.meta.days} (scatter)"
        )
